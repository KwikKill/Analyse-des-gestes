...
public class AnimationFileReader : MonoBehaviour
{
    ...
    public Action<List<Vector3>> notifyUpdatePos;
    ...
    private int _currentFrameIndex; //l'indice de la frame courrante
    ...
    private void Start()
    {
        ...
        {
            joinPrefab.GetComponent<TrailRenderer>().endWidth = 0.02f;
        }
        else
        {
    ...
    private void UpdateJoinPos()
    {
        ...
        if (notifyUpdatePos != null)
        {
            List<Vector3> trueCoordinates = currentFrameRenderer.coordonnes.Select(v3=>new Vector3(v3.x/Geste.MultiplierForVisualisation,
                v3.y/Geste.MultiplierForVisualisation,
                v3.z/Geste.MultiplierForVisualisation)).ToList();
            notifyUpdatePos(trueCoordinates);
        }
    ...
    private void OnIterationEnd()
    {
        ...
        {
            if(MenuNotificationGeneral.EndSequence!=null)
                MenuNotificationGeneral.EndSequence();
            play = false;
        }
        ResetJoinPos();
        ResetTrailPrefab();
