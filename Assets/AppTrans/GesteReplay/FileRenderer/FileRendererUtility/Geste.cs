...
public class Geste {
	...
	public static float MultiplierForVisualisation = 20;
	...
	private void GetFromFile(string filename)
	{
		...
		for(int index = 1; index<lines.Length - 1; ++index)
		{
			...
			{
				float posx = (float) Convert.ToDouble(posString[i], CultureInfo.InvariantCulture.NumberFormat) * MultiplierForVisualisation;
				float posy = (float) Convert.ToDouble(posString[i+1], CultureInfo.InvariantCulture.NumberFormat) * MultiplierForVisualisation;
				float posz = (float) Convert.ToDouble(posString[i+2], CultureInfo.InvariantCulture.NumberFormat) * MultiplierForVisualisation;
