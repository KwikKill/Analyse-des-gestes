    ...
    public abstract class MenuController<T>: MenuElement<T> where T : AppMenuItem
    {
        ...
        private IEnumerator InstanciateProviderWhenLaunched()
        {
            ...
            if (provider == null)
            {
                ...
                if (devicePrefabs.Provider is SimulatorProvider)
                {
                    GameObject pan = GameObject.Find("AssociationKeyGesture");
                    if (pan != null)
                        SimulatorLeapMotion.GetInstance().textForAssociationKeyGesture = pan.GetComponent<RectTransform>();
                }
            }
