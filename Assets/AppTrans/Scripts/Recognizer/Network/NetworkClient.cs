    ...
    public class NetworkClient
    {
        ...
        private void LoopWrite()
        {
            ...
            while (!_ExitLoop)
            {
                try
                {
                    ...
                    // send header
                    ProtoBuf.Serializer.SerializeWithLengthPrefix<Header>(_NetworkStream, lHeader, ProtoBuf.PrefixStyle.Fixed32);

                    // send data
                    switch (lHeader.objectType)
                    {
                        case eType.eFrame:
                            ProtoBuf.Serializer.SerializeWithLengthPrefix<Frame>(_NetworkStream, (Frame)lHeader.data, ProtoBuf.PrefixStyle.Fixed32);
                            break;
                        default:
                            break;
        ...
        private void LoopRead()
        {
            ...
            while (!_ExitLoop)
            {
                try
                {
                    Header lHeader = ProtoBuf.Serializer.DeserializeWithLengthPrefix<Header>(_NetworkStream, ProtoBuf.PrefixStyle.Fixed32);
                    if (lHeader == null) break;
                    if (lHeader.objectType == eType.eError)
                    {
                        ErrorMessage lErrorMessage = ProtoBuf.Serializer.DeserializeWithLengthPrefix<ErrorMessage>(_NetworkStream, ProtoBuf.PrefixStyle.Fixed32);
                    ...
                    {
                        FrameRecognitionResult frameRecognitionResult = ProtoBuf.Serializer.DeserializeWithLengthPrefix<FrameRecognitionResult>(_NetworkStream, ProtoBuf.PrefixStyle.Fixed32);
                ...
                catch (SocketException)
                {
                    ...
                    Console.WriteLine("Client disconnected. Reconnect the server...");
                }
